...
@Component({
  selector: 'app-registrovankorisnik',
  templateUrl: './registrovankorisnik.component.html',
  styleUrls: ['./registrovankorisnik.component.css']
})
export class RegistrovankorisnikComponent implements OnInit {
    ...
    if ((this.mincena == "") && (this.maxcena == "")) {
      this.nekretninaServis.dohvatiSveNekretnine("odobrena").subscribe((podaci: Nekretnina[]) => {
        ...
        this.nekretninaServis.dohvatiNekretnineRezPretrage(grad, min, max,"odobrena").subscribe((podaciN: Nekretnina[]) => {
          this.poruka = 'Rezultati pretrage:'
          ...
          if (podaciN.length == 0) this.poruka = 'Nema nekretnina sa trazenim parametrima'
